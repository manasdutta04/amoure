...
const MatchesPage = () => {
  ...
  const handleSwipeRight = (id) => {
    ...
    if (!isAlreadyMatched) {
      ...
      // Store latest match in localStorage (for demo purposes)
      try {
        const existingMatches = JSON.parse(localStorage.getItem('latestMatches') || '[]');
        existingMatches.unshift({
          id: matchedProfile.id,
          name: matchedProfile.displayName,
          timestamp: new Date().getTime()
        });
        localStorage.setItem('latestMatches', JSON.stringify(existingMatches.slice(0, 10))); // Keep only 10 latest
      } catch (error) {
  ...
  const handleMessage = (id) => {
    ...
    const matchedPerson = [...matches, ...potentialMatches].find(match => match.id === id);
    
    if (matchedPerson) {
      // Create a chat ID (in a real app, this would be generated by the backend)
      const chatId = `chat_${id}`;
      
      // Store match data in localStorage so we can access it in the Chat component
      const matchData = {
        id: chatId,
        userId: id,
      ...
      const existingMatches = JSON.parse(localStorage.getItem('activeMatches') || '[]');
      
      // Check if this match already exists
      const matchIndex = existingMatches.findIndex(match => match.id === chatId);
      
      if (matchIndex >= 0) {
        // Update existing match
        existingMatches[matchIndex] = matchData;
      } else {
        // Add new match
      ...
      localStorage.setItem('activeMatches', JSON.stringify(existingMatches));
